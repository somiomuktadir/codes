...
namespace Distributions {
    ...
    namespace Continuous {
        ...
        double normalPDF(double mean, double stdDev, double x) {
            if (stdDev <= 0) throw std::invalid_argument("Standard deviation must be positive.");
            return (1.0 / (stdDev * std::sqrt(2 * Utils::PI))) * std::exp(-0.5 * std::pow((x - mean) / stdDev, 2));
        }
        ...
        double gammaPDF(double k, double theta, double x) {
            ...
            if (x < 0) return 0.0;
            return (std::pow(x, k - 1) * std::exp(-x / theta)) / (std::pow(theta, k) * Utils::gammaFunction(k));
        }
        ...
        double chiSquarePDF(int k, double x) {
            ...
            if (x < 0) return 0.0;
            return (std::pow(x, (double)k / 2 - 1) * std::exp(-x / 2)) / (std::pow(2, (double)k / 2) * Utils::gammaFunction((double)k / 2));
        }
        ...
        double studentTPDF(int v, double x) {
            ...
            double num = Utils::gammaFunction((double)(v + 1) / 2);
            double den = std::sqrt(v * Utils::PI) * Utils::gammaFunction((double)v / 2);
            return (num / den) * std::pow(1 + (x * x) / v, -(double)(v + 1) / 2);
        }
        ...
        double studentTVariance(int v) {
            if (v <= 2) return std::numeric_limits<double>::infinity(); // Undefined for v<=2
            return (double)v / (v - 2);
        }
        ...
        double fDistributionPDF(int d1, int d2, double x) {
            ...
            double den = x * Utils::betaFunction((double)d1 / 2, (double)d2 / 2);
            return num / den;
        }
